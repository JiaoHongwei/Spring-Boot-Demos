package com.hw.springbootredisdistributedlock.util;

import com.hw.springbootredisdistributedlock.config.FastJsonRedisSerializer;
    ...
    public synchronized boolean lock() throws InterruptedException {
        ...
        while (timeout >= 0) {
            ...
            if (currentValueStr != null && Long.parseLong(currentValueStr) < System.currentTimeMillis()) {
                ...
                // 获取上一个锁到期时间，并设置现在的锁的到期时间
                // 只有一个线程才能获取上一个线上 的设置时间，因为 jedis.getSet是同步的
                if (oldValueStr != null && oldValueStr.equals(currentValueStr)) {
                    // 防止误删（覆盖，因为key是相同的）了他人的锁 ——这里达不到效果，这里值会被覆盖，但是因为什么相差了
                    // 很少的时间，所以可以接受
                    // 【分布式的情况下】：如果这个时候，多个线程恰好都到了这里，但是只有一个线程的设置值和当前值相同，
                    // 他才有权利获取锁
